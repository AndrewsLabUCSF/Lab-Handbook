# Understanding Unix/Linux File Permissions {.unnumbered}
...
If all the files you want to modify are in the same directory and follow a specific pattern, you can use a wildcard. Preview the matching files first so that you do not change unintended files.
...
- `printf` displays the files matched by the wildcard before anything is changed.
- `chmod 660` grants read and write access to the owner and group while denying access to others.
- `--` prevents a filename beginning with a hyphen from being interpreted as an option.

Compressed data files do not normally need execute permission. If the files should be readable but not writable by the group, use `chmod 640 -- *.txt.gz` instead. Adjust the wildcard only after confirming that it matches the intended files.
