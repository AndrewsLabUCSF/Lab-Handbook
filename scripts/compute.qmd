...
-   [Set up VS Code SSH forwarding](https://blog.wytamma.com/blog/hcp-vscode/): Allows you to access files on Wynton using VS Code
...
-   [Wynton Bioinformatics Open Data Resource](https://docpollard.org/resources/data/wynton-bioinformatics-open-data-resource/): A number of standard genomic datasets and resources are available so we do not need to download them.
...
Enter a strong passphrase when prompted, then enter it again to confirm it.
...
# Option 2: A more interactive way
...
Step 2: Authenticate 
 - ```gh auth login``` & answer the questions
    - Open https://github.com/login/device in a browser and enter the one-time code provided by the CLI.
- To verify the key pair works, type ```ssh -T git@github.com```. You should get a message confirming that you authenticated successfully and that GitHub does not provide shell access.
...
Type ```ssh -T git@github.com```. You should get a message confirming that you authenticated successfully and that GitHub does not provide shell access.
